# TASKS

상태 표기: `[ ]` 대기 / `[-]` 진행 / `[x]` 완료

## T01 프로젝트 초기화

- [x] .NET 8 WPF 솔루션/프로젝트
- [x] `.gitignore`
- [x] MIT License
- [x] README 및 기본 문서
- [x] Windows Release 빌드 검증

## T02 메인 UI Shell

- [x] 좌측 이미지 대화 목록 영역
- [x] 중앙 이미지 캔버스 영역
- [x] 우측 대화/프롬프트 영역
- [x] 상단 새 대화/설정 버튼
- [x] 하단 편집 모드 버튼
- [x] 앱 벡터 아이콘 및 창 아이콘 적용
- [x] 시안 기반 밝은 카드형 UI / 진한 상단 헤더 개편
- [x] 좌우 패널 GridSplitter 크기 조절
- [x] 좌우 패널 수동 접기/열기
- [x] 좁은 창에서 좌우 패널 자동 접기
- [x] 최소 창 크기 860×620으로 완화
- [-] 반응형 레이아웃 실기기 검증 대기
- [x] EXE 아이콘 자동 생성/컴파일 포함
- [ ] EXE 아이콘 실제 표시 확인

## T03 새 이미지 대화

- [x] 파일 선택 진입
- [x] Drag & Drop 진입
- [x] 중앙 이미지 표시 기본 구조
- [x] 이미지별 새 대화 생성
- [x] 원본 이미지를 대화 저장 폴더로 복사
- [x] 여러 대화를 좌측 목록에 누적 표시

## T04 로컬 대화 저장

- [x] SQLite 스키마
- [x] Conversation 저장/불러오기
- [x] Edit 저장/불러오기
- [x] 이미지 파일 버전 관리
- [x] 프로그램 재실행 시 복원
- [x] 앱 내부 저장(기존 방식)
- [x] 원본 폴더 아래 대화별 저장 폴더 생성
- [x] 특정 지정 폴더 아래 대화별 저장 폴더 생성
- [x] 생성 결과를 해당 대화 저장 폴더에 즉시 저장
- [x] 대화 삭제 시 이미지 폴더 삭제 여부 별도 확인
- [-] 외부 저장 위치 실기기 검증 대기

## T05 설정 창

- [x] 별도 설정 팝업
- [x] API Key 입력
- [x] Windows Credential Manager 저장 구조
- [x] 대표 모델 Selector
- [x] Custom Model ID
- [x] OpenRouter Image Models 컬렉션 링크
- [x] Grok Imagine Image 2.0 / Image Quality 프리셋
- [x] 수정 요청별 모델 Selector
- [x] 설정의 기본 모델을 요청 Selector 초기값으로 사용
- [x] Custom Model ID를 요청 Selector 마지막에 표시
- [x] 이미지 저장 위치 Selector
- [x] 특정 저장 폴더 선택 UI
- [x] 현재 API Key 사용량/잔여 한도 조회
- [x] Management Key인 경우 같은 키로 충전 잔액 조회
- [x] 추가 Management Key 설정 없이 현재 저장 키만 사용
- [ ] 실제 Windows 환경 저장/재실행 검증

## T06 OpenRouter 전체 이미지 편집

- [x] `/api/v1/images` 클라이언트 기본 구현
- [x] `input_references` base64 data URL 요청 구조
- [x] `data[0].b64_json` 응답 해석 구조
- [x] 메인 UI의 이미지 수정 버튼과 연결
- [x] 결과 이미지를 새 버전으로 저장
- [x] 요청/결과를 우측 대화 히스토리에 표시
- [x] 선택한 이전 결과 이미지를 다음 요청 입력으로 사용
- [x] OpenRouter 요청 도달 검증
- [x] 성공 이미지 응답 실사용 검증
- [x] 안전정책 차단 응답 한국어 안내
- [x] 인증/권한 오류 안내
- [x] 크레딧/결제 오류 안내
- [x] 429 요청 제한 안내
- [x] 5xx 공급자/서버 오류 안내

## T07 대화 목록/삭제

- [x] 최신 결과 썸네일
- [x] 파일명 대신 최신 프롬프트를 제목으로 표시
- [x] 프롬프트 앞뒤/연속 공백 정리
- [x] 최신 프롬프트 일부 표시
- [x] 마지막 작업 시각
- [x] 선택/전환
- [x] 가로 스크롤 제거
- [x] 삭제 확인
- [x] 대화 기록 삭제와 이미지 폴더 삭제를 분리

## T08 이미지 캔버스

- [x] 마우스 휠 줌 100~800%
- [x] 가운데 버튼 드래그 팬
- [x] 원본 픽셀 좌표 변환
- [x] 사각형 선택
- [x] 선택 영역 이동
- [x] 선택 영역 모서리 크기 변경
- [x] 선택 영역 투명 표시
- [x] Esc 선택 해제

## T09 영역 편집

- [x] 선택 영역 주변 문맥 50% 확장 Crop
- [x] OpenRouter 편집 요청
- [x] 실제 선택 영역만 추출
- [x] 원본 전체 크기 유지 합성
- [x] 선택 가장자리 feather blending
- [-] 선택 크기별 adaptive feather + 모서리 감쇠 개선 (구현, 실기기 결과 검증 대기)

## T10 Crop 편집

- [x] 선택 영역만 Crop
- [x] OpenRouter 편집 요청
- [x] 결과를 독립 이미지로 반환하고 원본에 재합성하지 않음
- [x] 실기기 기본 동작 검증

## T11 히스토리/버전 탐색

- [x] 요청별 결과 이미지 기록
- [x] 이전 결과 이미지 선택 및 메인 표시
- [x] 선택한 이전 결과를 기반으로 새 편집 시작
- [x] 기존 편집 작업 다시 시도(기존 결과 유지 + 새 결과 추가)
- [x] 재시도 시 당시 입력 이미지/프롬프트/편집방식/선택영역 재사용
- [x] 다시 시도별 모델 Selector
- [x] 다시 시도 Selector 기본값은 해당 작업의 기존 모델
- [x] 다시 시도 시 선택한 모델로 새 결과 추가
- [x] 재시도 모델 Selector 선택 중 LayoutUpdated UI 재구성 제거
- [x] 원본 비교
- [x] 원본 비교 토글 시 선택 영역 초기화
- [x] 현재 대화 텍스트 클릭 시 클립보드 복사

## T12 내보내기

- [x] PNG 변환 저장
- [x] JPG 변환 저장 (품질 95)
- [x] 현재 결과 원본 형식 저장

## T13 마무리 / 배포

- [x] 버전 0.5.0 메타데이터 적용
- [x] Windows x64 self-contained single-file Publish 프로필
- [x] `publish.ps1` 빌드 + Publish + ZIP 패키징
- [x] GitHub Actions main 자동 Windows 빌드/패키징
- [x] `release:` 커밋 또는 수동 실행 시 GitHub Release 자동 생성
- [x] GitHub Actions Windows x64 빌드/ZIP 패키징 성공 검증
- [x] v0.5.0 GitHub Release 자동 배포
- [ ] 배포 EXE 실기기 실행 검증
- [ ] 설정/API Key 저장 후 프로그램 재실행 검증
- [ ] Custom 모델 테스트
- [x] README 사용법 갱신
